...
void testPrimitiveOperations() {
    ...
    std::vector<int> v = {1,0,2,3,-4,10,23,-20,-3,4,4,4,4,3};
    SortedPQ<int> pq2(v.begin(), v.end(), std::less<int>());
    Eecs281PQ<int>& eecsPQ2 = pq2;

    assert(eecsPQ2.size() == 14);
    assert(eecsPQ2.top()  == 23);
    eecsPQ2.pop();
...
void testPairing() {
...

    {
        ...
        const std::vector<int> vec2 {
            1,0,2,6,5,4,3,0,0,-1,7,101
        };

        PairingPQ<int> pair1 { vec2.cbegin(), vec2.cend() };
        PairingPQ<int> pair2 { pair1 };
        PairingPQ<int> pair3 {};
        pair3 = pair2;

        Eecs281PQ<int>& p1 = pair1;
        // Eecs281PQ<int>& p2 = pair2;
        // Eecs281PQ<int>& p3 = pair3;
        ...
        assert(pair3.size() == 12);

        auto * it1 = pair3.addNode(1000);
        assert(pair3.size() == 13);
        assert(pair3.top() == 1000);

        auto * it2 = pair3.addNode(1001);
        assert(pair3.size() == 14);
        assert(pair3.top() == 1001);

        pair3.updateElt(it1, 1002);
        assert(pair3.size() == 14);
        assert(pair3.top() == 1002);

        pair3.updateElt(it2, 1003);
        assert(pair3.size() == 14);
        assert(pair3.top() == 1003);

        pair3.pop();
        pair3.pop();
        assert(pair3.top() == 101);

        auto * it3 = pair3.addNode(8);
        pair3.addNode(9);
        pair3.push(10);
        pair3.addNode(11);
        pair3.push(12);
        ...
        pair3.push(0);
        assert(pair3.top() == 101);
        pair3.updateElt(it3, 13);
        assert(pair3.top() == 101);
        pair3.updateElt(it3, 14);
        assert(pair3.top() == 101);

        pair3.updateElt(it3, 102);
        assert(pair3.top() == 102);
